...
renderCUDA(
	...
	// Tile1 (start, end)
	// ...
	const uint32_t* __restrict__ point_list, // 排序后的 3d gaussian 椭球 列表
	...
	const float* __restrict__ final_Ts, // forward render 后的最终透射率
	const uint32_t* __restrict__ n_contrib, // render中 用到的 gs 个数
	const float* __restrict__ dL_dpixels, // Loss function 对 每个像素 的梯度
	const float* __restrict__ dL_invdepths, // Loss function 对 深度倒数 的梯度
	// 待求
	float3* __restrict__ dL_dmean2D, // Loss function 对 2d gaussian坐标 的梯度
	float4* __restrict__ dL_dconic2D, // Loss function 对 协方差矩阵 (a, b, c) 的梯度
	float* __restrict__ dL_dopacity, // Loss function 对 基础不透明度 的梯度
	float* __restrict__ dL_dcolors, 
	float* __restrict__ dL_dinvdepths
)
{
	// We rasterize again. Compute necessary block info.
	// 和 forward renderCUDA 的流程一样 
	// 首先分配 pixel 给 thread 
	...
	// 得到的 range 有 range.x(当前tile中最近的那个gs在point_list中的编号) 和 range.y(当前tile中最远的那个gs在point_list中的编号)
	const int rounds = ((range.y - range.x + BLOCK_SIZE - 1) / BLOCK_SIZE);

	bool done = !inside;
	int toDo = range.y - range.x;
	// 这里还是为了避免显存带宽浪费
	// 使用共享内存的方式，让一个 block 中的所有 thread 一次性
	// 去显存中搬运 256 个高斯的前向状态到高速的共享内存中
	// 这里的共享内存里存的全部都是“前向传播的历史记录”，
	// 它们在反向传播过程中是“只读（Read-Only）”的，绝不会发生变化。
	...
	if (inside)
	{
		for (int i = 0; i < C; i++)
			...
			// dL_dpixels是由 Pytorch算完Loss后传给CUDA的
			// 一整张图片大小的梯度矩阵，存储在 Global VRAM 中。
			// 这里将每个像素对应的梯度存储到 GPU 的寄存器上 dL_dpixel
			// 也是为了加快后续计算梯度时读取 Loss Function 的速度
		if(dL_invdepths)
